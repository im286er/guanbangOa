...
if($submitMethod=='POST'){
	$grade1 = $pd->query("SELECT c.* from oa_zhsz_users u join oa_zhsz_class c on u.department=c.id where u.id={$_SESSION['admin_id']}")->fetchAll(PDO::FETCH_ASSOC);
	
	if(empty($grade1)){
		$tips = array(
		   'tips' => '毕业生数据错误，请连续管理员。',
		   'url'  => './?t=biye_info_export'
		);
		$tips = urlencode(serialize($tips));
		header('Location:../tips.php?gets='.$tips);
	}
	
	$gid = empty($grade1[0]['grade_id'])?3:$grade1[0]['grade_id'];
	...
	//查询班级
	$where  .= " and department IN ({$grade1[0]['id']}) ";	

	$where .= " and truename = '{$truename}' ";

	//查询当前学期
	if($tid != 10000){
		$curTerm = $pd->fetchRow(array('field'=>array('id','year','term_name','term_type','end'),'table'=>'oa_zhsz_term','where'=>"id={$tid}"));
		...
		$curTerm = $pd->fetchRow(array('field'=>array('id','year','term_name','term_type','end'),'table'=>'oa_zhsz_term','where'=>"flag_default=1"));
	...
	$fileName  = $_SERVER['DOCUMENT_ROOT'].'/oa/uploadfiles/down/'.$_SESSION['admin_id'].'/'.$filename.'.doc';
	//查询学生记录
	$users = $pd->query("SELECT id,username,truename,department,user_card,person_no,select_course_id from oa_zhsz_users  where {$where} order by CONVERT(truename USING GBK)")->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach($users as $rs){
		...
		$terms = $pd->query("SELECT id,year,term_name,term_type,end from oa_zhsz_term order by end")->fetchAll(PDO::FETCH_ASSOC);
		...
		if($tid==10000){
			...
			foreach($terms1 as $term){
				$gid_biye = intval("20".substr($grade1[0]['class_end'],0,2));
				$term_biye = intval(substr($term['end'],0,4));
				$tid_num = $gid_biye - $term_biye;
				...
				if(empty($main)){
					...
					if(!empty($class)){//有班级
						$main['class_name']  = $class['class_name'];
							$main['master_name'] = $pd->fetchOne(array('field'=>'truename','table'=>'oa_zhsz_users','where'=>"id={$class['class_master']}"));
					}else{
						$class = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_class','where'=>"id={$rs['department']}"));
						$main['class_name']  = $class['class_name'];
						$main['master_name'] = $pd->fetchOne(array('field'=>'truename','table'=>'oa_zhsz_users','where'=>"id={$class['class_master']}"));
					}
					//continue;
				...
				//查询研究性学习
				$research = $pd->fetchRow(array('field'=>array('subject','content','level','score'),'table'=>'oa_zhsz_research','where'=>"term_year='{$term['year']}' and create_by='{$rs['username']}' and flag_status='已审核'"));
				//查询社会服务
				$service = $pd->fetchRow(array('field'=>array('subject','content','level','score'),'table'=>'oa_zhsz_service','where'=>"term_id='{$term['id']}' and create_by='{$rs['username']}' and flag_status='已审核'"));
				//社会实践
				$event = $pd->fetchRow(array('field'=>array('subject','content','level','score'),'table'=>'oa_zhsz_event','where'=>"term_id='{$term['id']}' and create_by='{$rs['username']}' and flag_status='已审核'"));
				...
				if(!empty($event['subject'])){
					$len = iconv_strlen($event['subject'],'utf-8');
					$shhd = iconv_substr($event['subject'],0,$emax,'utf-8');
					if($len>$emax){
						$shhd .= '...';
					}
				...
				//评语
				$cmax = 180;
				if(empty($main['comment'])){
					$xqpy = '';
				}else{
					$len = iconv_strlen($main['comment'],'utf-8');
					$xqpy = iconv_substr($main['comment'],0,$cmax,'utf-8');
					if($len>$cmax){
						$xqpy .= '...';
					}
				}
			...
			if($tid==10000 && $gid==3 && $curTerm1['term_type']==2){     //导出总表
				...
				$fileC = str_replace('{classname}',$main1['class_name'],$fileC);
				$fileC = str_replace('{truename}',$rs['truename'],$fileC);
			...
			$gid_biye = intval("20".substr($grade1[0]['class_end'],0,2));
			$term_biye = intval(substr($curTerm['end'],0,4));
			$tid_num = $gid_biye - $term_biye;
			
			if($tid_num>3){
				$tips = array(
				   'tips' => '抱歉，当前学期对应的毕业届没有数据。',
				   'url'  => './?t=biye_info_export'
				);
				$tips = urlencode(serialize($tips));
				header('Location:../tips.php?gets='.$tips);
			...
			$score  = array(0=>$score1,1=>$score2);
			//查询研究性学习
			$research = $pd->fetchRow(array('field'=>array('subject','content','level','score'),'table'=>'oa_zhsz_research','where'=>"term_year='{$curTerm['year']}' and create_by='{$rs['username']}' and flag_status='已审核'"));
			//查询社会服务
			$service = $pd->fetchRow(array('field'=>array('subject','content','level','score'),'table'=>'oa_zhsz_service','where'=>"term_id='{$curTerm['id']}' and create_by='{$rs['username']}' and flag_status='已审核'"));
			//社会实践
			$event = $pd->fetchRow(array('field'=>array('subject','content','level','score'),'table'=>'oa_zhsz_event','where'=>"term_id='{$curTerm['id']}' and create_by='{$rs['username']}' and flag_status='已审核'"));
	...
	//插入下载文件到下载表中
	$data = array(
		'file' 		  => '/oa/uploadfiles/down/'.$_SESSION['admin_id'].'/'.$filename.'.doc',
		'name' 		  => $dataname,
		'flag_type'   => 12,
