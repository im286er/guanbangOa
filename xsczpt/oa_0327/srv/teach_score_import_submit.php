...
if($submitMethod=='POST'){
	...
	for($j=2;$j<=$highestRow;$j++){
		...
		$userId   = $user['id'];
		$cj 	  = $objPHPExcel->getActiveSheet()->getCell("D{$j}")->getValue();
		
		$yw  	  = $objPHPExcel->getActiveSheet()->getCell("C{$j}")->getValue();
		$sx 	  = $objPHPExcel->getActiveSheet()->getCell("P{$j}")->getValue();
		...
		//查询记录是否存在
		$e = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_score','where'=>"user_id='{$userId}' and term_id={$termId} and exam_type='{$et}' and ".$coursejp.">-1"));
