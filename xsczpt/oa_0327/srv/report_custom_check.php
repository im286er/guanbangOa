...
switch($_POST["tpl"]){
	case "set_checked":
		...
		if($parent_st==1){
			$parent_ids = $pd->query("select partner_id from oa_zhsz_report_custom where id='".$_POST["id"]."' and uid='".$_POST["geter"]."' GROUP BY create_by ")->fetchColumn(0);
			$w_time = $pd->query("select workgroup_id from oa_zhsz_report_custom where id='".$_POST["id"]."' and uid='".$_POST["geter"]."' GROUP BY create_by ")->fetchColumn(0);
			$parent_id = explode(',',$parent_ids);
            for($i=0;$i<count($parent_id);$i++){
                $rs=$pd->query("insert into oa_message(uid,geter,leaveword ,content,stime,is_read,flag,menu1,menu2,subject,from_id) values('".$uid."','".$parent_id[$i]."',(select content from oa_zhsz_report_custom where id=".$_POST["id"]."),'".$_POST["content"]."','".date('Y-m-d H:i:s')."',0,'".$_POST["flag"]."','".$_POST["menu1"]."','".$_POST["menu2"]."','".$_POST["subject"]."','".$_POST["id"]."')");
				$wcont=$pd->query("update oa_zhsz_report_custom SET flag_status='未通过' where workgroup_id='".$w_time."' ");
			}
			$rs1=$pd->query("insert into oa_message(uid,geter,leaveword ,content,stime,is_read,flag,menu1,menu2,subject,from_id) values('".$uid."','".$_POST["geter"]."',(select content from oa_zhsz_report_custom where id=".$_POST["id"]."),'".$_POST["content"]."','".date('Y-m-d H:i:s')."',0,'".$_POST["flag"]."','".$_POST["menu1"]."','".$_POST["menu2"]."','".$_POST["subject"]."','".$_POST["id"]."')");
	...
	case "set_tg":
		...
		if($parent_st==1){
			...
			$parent_id = explode(',',$parent_ids);
            for($i=0;$i<count($parent_id);$i++){
				$rs=$pd->query("insert into oa_message(uid,geter,leaveword ,stime,is_read,flag,menu1,menu2,subject,from_id) values('".$uid."','".$parent_id[$i]."',(select content from oa_zhsz_report_custom where id=".$_POST["id"]."),'".date('Y-m-d H:i:s')."',0,'".$_POST["flag"]."','".$_POST["menu1"]."','".$_POST["menu2"]."','".$_POST["subject"]."','".$_POST["id"]."')");
				$wcont=$pd->query("update oa_zhsz_report_custom SET flag_status='已审核' where workgroup_id='".$w_time."' ");
			}
			$rs1=$pd->query("insert into oa_message(uid,geter,leaveword ,stime,is_read,flag,menu1,menu2,subject,from_id) values('".$uid."','".$_POST["geter"]."',(select content from oa_zhsz_report_custom where id=".$_POST["id"]."),'".date('Y-m-d H:i:s')."',0,'".$_POST["flag"]."','".$_POST["menu1"]."','".$_POST["menu2"]."','".$_POST["subject"]."','".$_POST["id"]."')");
			if($_SESSION['role_id']!=4&&$_SESSION['role_id']!=1&&$_SESSION['role_id']!=3){
				$rs1=$pd->query("update oa_zhsz_report_custom SET master_id='".$uid."' where id=".$_POST["id"]."");
			}elseif($_SESSION['role_id']==4){
				$mid=$pd->query("select master_id from oa_zhsz_report_custom where id=".$_POST["id"]." ")->fetchColumn(0);
