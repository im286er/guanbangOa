...
if($submitMethod=='POST'){
	...
	if($op=='edit'){
		...
		if($flag_old=='未通过'){
			$rs=$pd->query("insert into oa_message(uid,geter,content,stime,is_read,flag,menu2,subject,from_id) values('".$uid."','".$_POST["geter"]."','".$_POST["content"]."','".date('Y-m-d H:i:s',time())."',0,'待审核','55',(select flag_type from oa_zhsz_reward_punishment where id=".$id."),{$id})");
			//添加
			$data = array(
				'flag_status'     => "待审核"
			);
	...
	if(empty($result)){
		...
		if($next!=1){
		   ...
		   $rs=$pd->query("insert into oa_message(uid,geter,content,stime,is_read,flag,menu2,subject,from_id) values('".$uid."','".$_POST["geter"]."','".$_POST["content"]."','".date('Y-m-d H:i:s',time())."',0,'".$status."','55',(select flag_type from oa_zhsz_reward_punishment where id=".$biye_id."),{$biye_id})");	
		}
		//$biye_id = $pd->fetchOne(array('field'=>'max(id)','table'=>'oa_zhsz_reward_punishment'));
		//$rs=$pd->query("insert into oa_message(uid,geter,content,stime,is_read,flag,menu2,from_id) values('".$uid."','".$_POST["geter"]."','".$_POST["content"]."','".date('Y-m-d H:i:s',time())."',0,'".$status."','55',{$biye_id})");
		$tips = urlencode(serialize($tips));
		header('Location:../tips.php?gets='.$tips);
	}
}else{
