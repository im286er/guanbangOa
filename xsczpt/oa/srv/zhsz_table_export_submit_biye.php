...
set_time_limit(0);
	...
	if($submitMethod=='POST'){
		//是否为班主任
		$isMaster = false;
		$class = $pd->fetchRow(array('field'=>array('id','grade_id'),'table'=>'oa_zhsz_class','where'=>"class_master={$_SESSION['admin_id']} and grade_id<>0"));
		...
		if(!$gid){
			...
			$gid_biye = intval("20".substr($gid,0,2));
			$tid_biye = intval(substr($curTerm['end'],0,4));
			$gid_num = $gid_biye - $tid_biye;
			if($gid_num == 0){
				$grade_name = "高三";
				$grade_id = 3;
			}else if($gid_num == 1){
		...
		$grade     = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_grade','where'=>"id={$grade_id}"));
		$gradeName = $gid.$grade['grade_name'];
		//查询班级
		if(!empty($cid)){
			$class     = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_class','where'=>"id={$cid}"));
			$className = $class['class_name'];
			$where .= " and department={$cid} ";
		}else{
			$classId = $pd->fetchCol(array('field'=>'id','table'=>'oa_zhsz_class','where'=>"grade_id={$grade_id}"));
			$classId = join(',',$classId);
			$where   .= " and department IN ({$classId}) ";	
		}
		...
		//查询学生记录
    $users = $pd->query("select id,username,truename,department,user_card,person_no,select_course_id from oa_zhsz_users where ".$where." order by CONVERT(truename USING GBK) desc")->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach($users as $rs){
		...
		$uid = $rs['id'];
		//查询主表
		$main = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_main','where'=>"user_id={$uid} and term_id={$curTerm['id']} and class_id={$rs['department']} "));
		if(empty($main)){
			$main = array();
			$main['id'] = -1;
			if(!empty($class)){//有班级
      ...
      $level = $pd->query("select course_id,level,checker from oa_zhsz_course_level where user_id={$uid} and term_id={$curTerm['id']}")->fetchAll(PDO::FETCH_ASSOC);
			...
			//查询体质记录
			$physique = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_physique','where'=>"user_id={$uid} and term_year='{$curTerm['year']}'"));
  ...
  style='font-size:9.0pt;font-family:宋体'>".(empty($score[1][$rsb['course_code'].'_xf'])||$score[1][$rsb['course_code'].'_xf']==-1?'':$score[1][$rsb['course_code'].'_xf'])."<o:p></o:p></span></p>
   ...
   <tr style='mso-yfti-irow:0;mso-yfti-firstrow:yes;mso-yfti-lastrow:yes;
    ...
    //插入下载文件到下载表中
		/*$data = array(          
						'file' 		  => '/uploadfiles/down/'.$_SESSION['admin_id'].'/'.$filename.'.doc',
						'name' 		  => $dataname,
						'flag_type'   => 12,
    ...
    $create_time = date('Y-m-d H:i:s');
    $pd->exec("insert into oa_zhsz_table_downs(file,name,flag_type,create_by,create_time) values('/oa/uploadfiles/down/".$_SESSION['admin_id']."/".$filename.".doc','".$dataname."',12,'".$_SESSION['admin_name']."','".$create_time."')");
    //直接下载
		downloadFile($fileName,$filename.'.doc');
   // echo '/uploadfiles/down/'.$_SESSION['admin_id'].'/'.$filename.'.doc';
		exit;
	}else{
		//提交方式出错
		$tips = array(
