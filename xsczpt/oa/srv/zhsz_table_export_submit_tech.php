...
if($submitMethod=='POST'){
	...
	//读取模板文件
	$template = readFileContent('zhsz_template_new.html');
	...
	foreach($users as $rs){
		...
		if($tid==10000){
			...
			//查询主表
			$main = $pd->fetchRow(array('field'=>array('*'),'table'=>'oa_zhsz_main','where'=>"user_id='{$uid}' and term_id={$tid}"));
			$is_total_name = 0;
			if(empty($main)){
				$main = array();
				$main['id']  = -1;
			...
			if(empty($content['content'])){
				...
				$len = iconv_strlen($main['content'],'utf-8');
				$xqpy = iconv_substr($main['content'],0,$cmax,'utf-8');
				if($len>$cmax){
					$xqpy .= '...';
				}
				if(!$content['master_name'])
