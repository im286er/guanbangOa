...
if($submitMethod=='POST'){
	...
	$priv = empty($_POST['priv'])?"":$_POST['priv'];
	...
	if($_SESSION['role_id']==4||$_SESSION['role_id']==5||$_SESSION['role_id']==6){
		...
		//echo "111";
		$class_d=$pd->fetchOne(array('field'=>'dept','table'=>'v_users','where'=>"id='".$_POST["uid"]."'"));
		//echo "222";
		$geterid = $pd->fetchOne(array('field'=>'class_master','table'=>'oa_zhsz_class','where'=>"id=".$class_d.""));
		$fzt = '待审核';
		//exit;
	}
	...
	$rs=$pd->query("insert into oa_message(uid,geter,content,stime,is_read,flag,menu2,from_id) values('".$_POST["uid"]."','".$geterid."','".$_POST["content"]."','".date('Y-m-d H:i:s',time())."',0,'".$fzt."','34',{$biye_id})");
	/* echo "insert into oa_message(uid,geter,content,stime,is_read,flag,menu2,from_id) values('".$_POST["uid"]."','".$geterid."','".$_POST["content"]."','".date('Y-m-d H:i:s',time())."',0,'".$fzt."','34',{$biye_id})";
	exit; */
	$tips = urlencode(serialize($tips));
	header('Location:../tips.php?gets='.$tips);
	exit;
