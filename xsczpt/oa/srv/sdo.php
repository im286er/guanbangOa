...
switch($action){ 
	...
	case 'del_hope' :
		$id = $filter->filter_number($_POST['id']);
		//删除信息
		$params = array('table'=>'oa_zhsz_hope','where'=>"id={$id}");
		if($pd->delete($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '期望删除成功。';
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '期望删除失败。';
		}
	...
	case 'clear_class' :
		//清空原有班级信息
		$r = $pd->exec("update oa_user_extend set org_class=dept,dept=0 where flag_type=1 and dept>0");
		if($r==true){
			$returnVal['flag']  = 'true';
	...
	//查询自定义报表
	/*case 'select_report_all' :
		$rs  = $pd->query("SELECT * from  oa_zhsz_report where parent_no='' and school={$_SESSION['school']} order by order_value");
	...
	case 'content_edit' :
		$id = $filter->filter_number($_POST['id']);
		$content  = $pd->query("SELECT A.*,B.truename from  oa_zhsz_teacher_comment as A inner join act_member as B on A.user_id=B.id where A.id={$id}")->fetchAll(PDO::FETCH_ASSOC);
	...
	//他人举报编辑审核
	case 'other_replay_edit' :
		$id = $filter->filter_number($_POST['id']);
		$content  = $pd->query("SELECT * from  oa_zhsz_other_report where id={$id}")->fetchAll(PDO::FETCH_ASSOC);
	...
	case 'del_meb_user' :
		$id = $_POST['id'];
		//删除信息
		$params = array('table'=>'act_member','where'=>"id='{$id}'");
		if($pd->delete($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '记录删除成功。';
		}else{
	...
	case 'reset_pass' :
		...
		$role_id = $pd->fetchOne(array('field'=>'role_id','table'=>'oa_user_extend','where'=>"userid='{$id}'"));
		if($role_id!=1){
			$userpass = '123456';
		}else{
			$userpass = substr($username,-6);
		}
		$data = array('pmd5'=>md5($userpass));

		$params = array('table'=>'act_member','data'=>$data,'where'=>"id='{$id}'");
	...
	case 'check_vuser' :
		$schoolname = Filter::filter_html($_POST['schoolname']);
		$where    = "school='{$schoolname}'";
		$params   = array('field'=>'count(*)','table'=>'acade_info','where'=>"{$where}");
		$nums     = $pd->fetchOne($params);
		
		if($nums>0){
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '抱歉，该学校已存在。';
		}else{
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '恭喜，该学校可以使用。';
		}
		echo json_encode($returnVal);
		break;
	...
	case 'delSkill' :
		...
		$tid = $filter->filter_number($_POST['tid']);
		$where = '';
		if($_SESSION['role_id']!=4)     //管理员删除全部，任课老师删除对应科目的记录
			$where = " and checker_id='{$_SESSION['uid']}'";
		//删除信息
		$params = array('table'=>'oa_zhsz_course_level','where'=>"user_id='{$uid}' and term_id='{$tid}' {$where}");

		if($pd->delete($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '技能素质删除成功。';
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '技能素质删除失败。';
		}
	...
	case 'del_status' :
		$id     = $filter->filter_number($_POST['id']);
		//删除信息
		$params = array('table'=>'oa_zhsz_status_manage','where'=>"id={$id}");
		if($pd->delete($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '学籍删除成功。';
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '学籍删除失败。';
		}
	...
	case "select_acade":
		$id = $filter->filter_number($_POST['aid']);
		$where  = " pid={$id} ";
		
		$class=$pd->query("select id,name from career_acade_feature where".$where)->fetchAll(PDO::FETCH_ASSOC);
	...
	case "r_cate":
		$id = $filter->filter_number($_POST['id']);
		$params = array('table'=>'major_cate','where'=>"id = {$id}");
		if($pd->delete($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '分类删除成功。';
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '分类删除失败。';
		}
