...
if($submitMethod=='POST'){
	...
	if(!empty($subject_id)&&!empty($content)){
	    ...
	    #记录这条分类下的类型的积分值
		$p_sum = $pd->fetchOne(array('field'=>'point_value','table'=>'oa_zhsz_report','where'=>'id='.$sub_id.''));
		$select_sum = $pd->fetchOne(array('field'=>'point_sums','table'=>'act_member','where'=>'id="'.$uid.'"'));
		#判断该学生在本学期的该分类下在表中是否有记录
		/* if($subject_son_id){
			$s_son='subject_son_id';
		}else{
			$s_son='subject_id';
		} */
		
		$member = $pd->fetchOne(array('field'=>'point_sum','table'=>'oa_zhsz_point','where'=>'term_id='.$curTerm['id'].' and uid="'.$uid.'" and subject_id='.$subject_id.''));
		if($member==false){
			$rs=$pd->query("insert into oa_zhsz_point(uid,term_id,subject_id,point_sum) values('".$uid."',".$curTerm['id'].",'".$subject_id."',".$p_sum.")");
		}else{
			$rs=$pd->query("update oa_zhsz_point set point_sum=point_sum+".$p_sum." where term_id=".$curTerm['id']." and uid='".$uid."' and subject_id=".$subject_id."");
		}
		
		if(empty($select_sum)){
			$rs=$pd->query("update act_member set point_sums=".$p_sum." where id='".$uid."'");
		}else{
			$rs=$pd->query("update act_member set point_sums=point_sums+".$p_sum." where id='".$uid."'");
		}
		if($partner_id){   //为合作人添加同样的记录
			...
			foreach($partner_arr as $rs){
				...
				#记录这条分类下的类型的积分值
				$p_sum = $pd->fetchOne(array('field'=>'point_value','table'=>'oa_zhsz_report','where'=>'id='.$sub_id.''));
				$select_sum = $pd->fetchOne(array('field'=>'point_sums','table'=>'act_member','where'=>'id="'.$rs.'"'));
				if(empty($select_sum)){
					$rs=$pd->query("update act_member set point_sums=".$p_sum." where id='".$rs."'");
				}else{
					$rs=$pd->query("update act_member set point_sums=point_sums+".$p_sum." where id='".$rs."'");
				}
				#判断该学生在本学期的该分类下在表中是否有记录
				/* if($subject_son_id){
					$s_son='subject_son_id';
				...
				//$member = $pd->fetchOne(array('field'=>'point_sum','table'=>'oa_zhsz_point','where'=>'term_id='.$curTerm['id'].' and uid="'.$rs.'" and '.$s_son.'='.$sub_id.''));
				//echo $rs;
				$member = $pd->fetchOne(array('field'=>'point_sum','table'=>'oa_zhsz_point','where'=>'term_id='.$curTerm['id'].' and uid="'.$rs.'" and subject_id='.$subject_id.''));
				//echo $member;
				//subject_id=".$subject_id."
				if($member==false){
					$rs=$pd->query("insert into oa_zhsz_point(uid,term_id,subject_id,point_sum) values('".$rs."',".$curTerm['id'].",'".$subject_id."',".$p_sum.")");
				}else{
					$rs=$pd->query("update oa_zhsz_point set point_sum=point_sum+".$p_sum." where term_id=".$curTerm['id']." and uid='".$rs."' and subject_id=".$subject_id."");
				}
