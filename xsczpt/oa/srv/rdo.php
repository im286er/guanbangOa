...
switch($_POST["tpl"]){
	// case "get_level":
	// 	$pid = $_POST['pid'];
	// 	$rs=$pd->query("select level from major_cate where id = {$pid}");
	// 	echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
	// 	break;  

	case "get_terms":
		$rs=$pd->query("select id,year,term_name from oa_zhsz_term order by year desc,term_type desc");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break;   
	...
	case "aca_sub":
		$rs=$pd->query("select id,name from career_acade_type");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break; 

    case "acade":
		$rs=$pd->query("select id,name from career_acade_level");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break;
    		
    case "nature":
		$rs=$pd->query("select * from career_acade_nature");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break;	
		
	case "aca_addr":
		$rs=$pd->query("select * from career_acade_addr order by odx asc");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break;
	...
	case "check_other_report":
		$id  = $filter->filter_number($_POST['id']);
		$val = $filter->filter_number($_POST['val']);
		$status = $filter->filter_number($_POST['status']);
		$reason = $filter->filter_html($_POST['reason']);
		//审核信息
		$data = array('flag_checked'=>$val,'flag_status'=>$status,'reason'=>$reason);
		$params = array('data'=>$data,'table'=>'oa_zhsz_other_report','where'=>"id={$id}");
		if($pd->update($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '操作成功。';
			$zt = $pd->fetchOne(array('field'=>'flag_status','table'=>'oa_zhsz_other_report','where'=>"id={$id}"));
			if($zt==2){
				$flag_status = '审核通过';
			}else if($zt==3){
				$flag_status = '审核不通过';
			...
			$rs=$pd->query("insert into oa_message(uid,geter,leaveword,content,stime,is_read,flag,menu2,subject,from_id) values('".$uid."',(select id from v_users where username=(select create_by from oa_zhsz_other_report where id={$id})),(select content from oa_zhsz_other_report where id={$id}),(select reason from oa_zhsz_other_report where id={$id}),'".date('Y-m-d H:i:s',time())."',0,'".$flag_status."','52','我要举报','{$id}')");
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '操作失败。';
		}
	...
	case "class_manage": #班级管理
		...
		foreach($class as $k=>$rs){
			...
			if(!empty($rs['class_master'])){
				$classMaster = $pd->fetchOne(array('field'=>'truename','table'=>'v_users','where'=>"id='{$rs['class_master']}'"));	
			}
	...
	case "report_del":
		$id   = $filter->filter_number($_POST['id']);
		//删除信息
		$params = array('table'=>'oa_zhsz_report','where'=>"id={$id}");
		if($pd->delete($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '报表项删除成功。';
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '报表项删除失败。';
		}
		echo json_encode($returnVal);
		break;
	...
	case "message_edit":
		$id   = $filter->filter_number($_POST['id']);
		$rs=$pd->query("select * from oa_message where id={$id}");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break;
    case "apply_check":
		$id  = $filter->filter_number($_POST['id']);
		$status = $filter->filter_number($_POST['status']);
		$reason = $filter->filter_html($_POST['reason']);
		$subject = $pd->fetchOne(array('field'=>'subject','table'=>'oa_zhsz_report_custom','where'=>"id=".$id.""));
		if($status==2){
			$apply_status="<span style=\'color:blue;\'>(申请通过)</span>";
			$subject .= $apply_status;
			$pd->query("update oa_zhsz_report_custom SET flag_status='待审核',re_submit=0 where id=".$id." ");
			$rs=$pd->query("insert into oa_message(uid,geter,content,leaveword,stime,is_read,flag,menu1,menu2,subject,from_id) values('".$uid."',(select uid from oa_zhsz_report_custom where id=".$id."),'".$reason."',(select content from oa_zhsz_report_custom where id=".$id."),'".date('Y-m-d H:i:s')."',0,'待审核','".$_POST["menu1"]."','".$_POST["menu2"]."','".$subject."','".$id."')");
		}else{
			$apply_status="<span style=\'color:blue;\'>(申请不通过)</span>";
			$subject .= $apply_status;
			$pd->query("update oa_zhsz_report_custom SET re_submit=0 where id=".$id." ");
			$rs=$pd->query("insert into oa_message(uid,geter,content,leaveword,stime,is_read,flag,menu1,menu2,subject,from_id) values('".$uid."',(select uid from oa_zhsz_report_custom where id=".$id."),'".$reason."',(select content from oa_zhsz_report_custom where id=".$id."),'".date('Y-m-d H:i:s')."',0,'申请不通过','".$_POST["menu1"]."','".$_POST["menu2"]."','".$subject."','".$id."')");
		}
		echo "ok";
		break;

    case "check_other_remarks":
		$id  = $filter->filter_number($_POST['id']);
		$val = $filter->filter_number($_POST['val']);
		$status = $filter->filter_html($_POST['status']);
		$reason = $filter->filter_html($_POST['reason']);
		//审核信息
		$data = array('flag_checked'=>$val,'flag_status'=>$status,'reason'=>$reason);
		$params = array('data'=>$data,'table'=>'oa_zhsz_remarks','where'=>"id={$id}");
		if($pd->update($params)){
			$returnVal['flag']  = 'true';
			$returnVal['value'] = '操作成功。';
			$zt = $pd->fetchOne(array('field'=>'flag_status','table'=>'oa_zhsz_remarks','where'=>"id={$id}"));
			$rs=$pd->query("insert into oa_message(uid,geter,leaveword,content,stime,is_read,flag,menu2,subject,from_id) values('".$uid."',(select id from v_users where username=(select create_by from oa_zhsz_remarks where id={$id})),(select content from oa_zhsz_remarks where id={$id}),(select reason from oa_zhsz_remarks where id={$id}),'".date('Y-m-d H:i:s',time())."',0,'".$zt."','67','友朋寄语','{$id}')");
		}else{
			$returnVal['flag']  = 'false';
			$returnVal['value'] = '操作失败。';
		}
		...
		break;
    case "student_status":
		$rs=$pd->query("select * from oa_zhsz_status_manage");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
		break;
