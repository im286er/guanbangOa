...
{
	$data = array();
	$termN = array('1'=>'初一上','2'=>'初一下','3'=>'初二上','4'=>'初二下','5'=>'初三上','6'=>'初三下');
	$curTerm = $db->fetchRow(array('field'=>array('id','year','term_name','start','end','term_type'),'table'=>'oa_zhsz_term','where'=>'flag_default=1'));
	
	$department = $db->fetchOne(array('field'=>'dept','table'=>'oa_user_extend','where'=>"userid='{$userId}'"));

	$gradeId    = $db->fetchOne(array('field'=>'grade_id','table'=>'oa_zhsz_class','where'=>"id={$department} and school={$_SESSION['school']}"));
	...
	//查询学期
	$term = $db->fetchAll(array('field'=>array('id','year','term_type'),'table'=>'oa_zhsz_term','where'=>"end<='{$curTerm['end']}'",'order'=>'year desc,term_type desc','page'=>1,'nums'=>$n));
