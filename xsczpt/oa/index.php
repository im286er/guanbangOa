...
switch($qname){
	case "index": #oa首页 
	    $xj_s = $pd->fetchOne(array('field'=>'xj_status','table'=>'oa_user_extend','where'=>"userid='{$_SESSION['uid']}'"));
		...
		elseif($_SESSION['role_id']==1){
			if($xj_s!=3){
				...
				if(empty($curTerm)){
					$tips = array(
					   'tips' => "抱歉，您已从该学校转出,已停用您在该平台的服务！",
					   'url'  => 'http://www.hazx.org.cn/'
					);
	...
	case "top_admin": #top   
		...
		// $str_menu .= '<li><a href="?t=career_plan" target="main">生涯规划</a><span></span></li>';
		foreach($menus as $rs){
			$menu_son = $pd->query("select * from oa_features where m_id={$rs['id']}")->fetchAll(PDO::FETCH_ASSOC);
			if($menu_son){
				$str_menu .= '<li><a href="?t=left&id='.$rs['id'].'" target="lef_m">'.$rs['fea_name'].'</a><span></span></li>';
		...
		if($_SESSION['role_id']==1){
		  if($xj_s!=3){
			...
			foreach($menus as $rs){
			  ...
			  $str_menu = '';
			  echo "<h2 style='color:red;align-text:center;padding-left:400px;padding-top:60px;'>抱歉，您已从该学校转出。</h2>";
		  }	
		}else{
		...
		foreach($main as $rs){
			...
			if($atts!=null){
				foreach($atts as $att){
					if($att['type']==1&&$att['attachment']!=null){
						//$att['attachment']=substr($att['attachment'],3);
			...
			foreach($report_item as $rsr){
				...
				foreach($item_array as $ita){
					$array = explode(":",$ita);
					//$rsr_value=$array[0]."|".$rsr['id'];
					if($array[0] == $rsr['id']){
						$rsr_value = base64_decode($array[1]);
						//$rsr_value = mb_convert_encoding($rsr_value, 'utf-8');
					}
				}
				$str_custom .= '<tr><td width="15%" class="tabtd2_L hat_name" style="text-align:center;font-weight:bold;">'.$rsr['item_name'].'</td><td width="85%" class="tabtd2_L hat_name" style="text-align:center;border-right:0px;">'.$rsr_value.'</td></tr>';
				
			} */
			
			$str_main .= '<li style="margin:-17px 1px 15px 1px;">
				<table border="0" class="tab ha_ret_tit" cellpadding="0" cellspacing="0" style="background:#a3c0f9!important;">
					<tbody>
						...
						<td class="tabtd2_L w8" style="width:150px;">'.$rs['subject'].'</td>
						<td class="tabtd2_L hat_name">子类</td>
						<td class="tabtd2_L w8" style="width:150px;">'.$rs['subject_son'].'</td>
						<td class="tabtd2_L hat_name">年级</td>
						<td class="tabtd2_L w49">'.$grade.'</td>
						<td class="tabtd2_L w8" style="width:150px;">操作</td></tr>
					</tbody>
				</table>
				<table border="0" class="tab" cellpadding="0" cellspacing="0" style="border-top:none;">
					  <tbody>
						  <tr>
							...
							<td colspan="" rowspan="" class="tabtd2_R" style="text-align:center; padding-top:5px;border-bottom:none;position:relative;padding-top:6%;border-top:#45c8dc 1px solid;width:150px;">'.$if_edit.'</td>
						  </tr>
					</tbody>
				</table>
			</li>';
   ...
   case "message_teacher":
	...
	case "report_custom_check":  //自定义评价审核
		...
		$str_main = '';
		$main = $pd->query("select A.* ,B.truename,C.dept from oa_zhsz_report_custom as A inner join act_member as B on(A.uid=B.id) inner join oa_user_extend as C on(B.id=C.userid) where {$where} and A.subject_son<>'学期计划' order by CONVERT(B.truename USING GBK)  limit {$offset},20")->fetchAll(PDO::FETCH_ASSOC);
				...
				<table border="0" class="tab" cellpadding="0" cellspacing="0" style="border-top:none;">
					  <tbody>
						  ...
						  <tr>
							<td width="" class="tabtd2_L hat_name" style="text-align:center;border-bottom:0;">内容</td>
							<td width="" class="tabtd2_R ha_tabconw" style="height:250px;overflow:auto;border-bottom:0;"><a target="" style="display:inline-block;height:250px;overflow:auto;line-height:20px;">'.$rs['content'].'</a></td>
							<td colspan="" width="" rowspan="2" class="tabtd2_R ha_taphoto" style="text-align:center;border-bottom:0;border-top:#45c8dc 1px solid;">
								<dl class="ha_forimg2" style="display:inline-block;">'.$att_pic.'</dl>
							&nbsp;</td>
							<td colspan="" rowspan="" class="tabtd2_R" style="text-align:center; padding-top:5px;border-bottom:none;position:relative;padding-top:6%;border-top:#45c8dc 1px solid;">
	...
	case "school_info":
		//学院层次查询
		// 搜索筛板数据
		// $T->SetBlock("acade_level","select * from carrer_acade_level");
		// $T->SetBlock("acade_addr",'select * from carrer_acade_addr');
		// $T->SetBlock("acade_feature",'select * from carrer_acade_feature');
		// $T->SetBlock("acade_nature",'select * from carrer_acade_nature');
		// $T->SetBlock("acade_type",'select * from carrer_acade_type');
		
		// // $T->SetBlock("acade_detail",'select * from carrer_acade_detail');

		// // 院校具体信息数据
		// $T->SetBlock("acade_info",'select * from carrer_acade_info');
	$T->SetBlock2("career_acade_list_title","select * from career_acade_cate  where level=1 order by id",
			array(array("block"=>"carrer_acade_cate_list","pid"=>"id","sql"=>"select * from career_acade_cate where pid=?")));

	// 院校详情信息查询
	$T->SetBlock("career_acade_info",'select * from career_acade_info');
	// $sql = 'select * from career_acade_info';
	// $school_info=$pd->query($sql)->fetchAll(PDO::FETCH_ASSOC);
	// print_r($school_info);
	// $str = "";
	// foreach($school_info as $vo){
	// $school_id = $vo['id'];
	// $string_985 = $vo['is_985']?'<span class="tag red">985</span>':"";
	// $string_211 = $vo['is_211']?'<span class="tag orange">211</span>':"";
	// $school_name = $vo['school'];
	// $school_addr = $vo['addr'];
	// $school_nature = $vo['nature'];
	// $school_belong = $vo['belong'];
	// $school_order = $vo['order'];
	
	// $str .= '<li class="clearfix">
 //                <a href=""  class="addRead"><img src="image/carrer_image/'."$school_name".'.jpg" alt="" class="collegesLogo fl"></a>
 //                <p class="compareDetail fr">
 //                  <a href="http://localhost:883/oa/?t=school_detail&id='."$school_id".'" target="_self"  class="addRead">查看详情</a>
 //                </p>
 //                <div class="collegeFeature">
 //                  <h3 class="mrgB10">
 //                        <a href="" class="addRead">'."$school_name".'</a>

 //                        '."$string_985 "."$string_211 ".'

 //                  </h3>
 //                  <i class="Lico inline-block mrgR5"></i>'."$school_addr".'<span class="mrgL25 mrgR25">'."$school_nature".'</span><span class="mrgR25">隶属：'."$school_belong".'</span>全国排名：'."$school_order".'                
	// 			</div>
 //             </li>';
 //    }
 //    $T->Set("str",$str);
	...
	case 'school_detail':
		...
		//var_dump($_GET);die;
		$id = $_GET['id'];
		$sql = "select * from career_acade_info where id = '{$id}' limit 1";
		$school_infos=$pd->query($sql)->fetchAll(PDO::FETCH_ASSOC);
		$school_info = $school_infos['0'];
		//$T->Set("$school_info",$school_infos['0']);
		// 模板赋值
		foreach ($school_infos['0'] as $key => $value) {
			$T->Set("$key" , $value);
		}
		print_r($school_infos);
		// 历年分数线拼装
		$recruiting_schemes = json_decode($school_info['je_re_scheme'],true);
		//var_dump($recruiting_schemes);
		$plan_table = "";
		for ($i=0; $i < count($recruiting_schemes['plan_name']); $i++) { 
			$plan_table .= '
				  <table class="applyTableData mrgB5">
                    <tbody><tr>
                        <td width="40%">
                            <a href="http://www.ewt360.com/Colleges/MajorDetail/34505?CollegeID=1530">'.$recruiting_schemes['major_name']["$i"].'</a>
                        </td>
                        <td width="15%">'.$recruiting_schemes['plan_name']["$i"].'</td>
                        <td width="15%">'.$recruiting_schemes['special']["$i"].'</td>
                        <td width="15%">'.$recruiting_schemes['plan_num']["$i"].'</td>
                        <td width="15%">'.$recruiting_schemes['year']["$i"].'</td>
                    </tr>
                </tbody></table>
			';
		}
		$T->Set("plan_table" , $plan_table);
		//echo "$plan_table";

		// 招生计划拼装
		// [je_year_score] => {"year":["2016"],"pici":["一本"],"kenei":["文科"],"lowest_score":["407"],"s_level":["A+A"]}
		$pass_score = json_decode($school_info['je_year_score'],true);
		//var_dump($pass_score);
		$score_table = "";
		for ($i=0; $i < count($pass_score['year']); $i++) { 
			$score_table .= '
	                <table class="applyTableData mrgB5">
                    <tbody><tr>
                        <td width="12%">'.$pass_score['year']["$i"].'</td>
                        <td width="12%">'.$pass_score['pici']["$i"].'</td>
                        <td width="10%">'.$pass_score['kenei']["$i"].'</td>
                        <td width="12%">'.$pass_score['lowest_score']["$i"].'</td>
                            <td width="12%">'.$pass_score['s_level']["$i"].'</td>
                    <td>&nbsp;</td>
                    </tr>
                </tbody></table>
			';
		}
		$T->Set("score_table" , $score_table);
		echo "$score_table";
		break;
	// 历年分数线
	case 'pass_score':
		break;
	// 招生计划
	case 'recruiting_scheme':
		# code...
		break;
	case 'major_cate_add':
		break;

	case 'major_cate_show':
		$T->SetBlock("cates",'select id,name as cate_name,fid as f_id, level as cate_level,pic as cate_pic from major_cate');
		break;

	case 'major_cate_edit':
		if(!empty($_GET['id'])){
			$id = $_GET['id'];
			$data = $pd->query("select id,name as cate_name,fid as pid, level as cate_level,pic as cate_pic from major_cate where id = {$id} limit 1")->fetchAll(PDO::FETCH_ASSOC);
			//var_dump($data['0']);die;
			//$T->Set("cate_name",$data['0']["cate_name"]);
...
//
//		$T->Set("cate_name",$cate_name);
//		$T->Set("cate_level",$cate_level);
//		$T->Set("cate_img",$cate_name);
//		$T->Set("pid",$pid);
		break;
	...
	case "stamina_rs_manage": #体能管理
		...
		/* if($gid>0){
			...
			if($cid==0){
				...
				$classId  = join(',',$classId);
				//$cid = $T->db->query('select id from oa_zhsz_class where grade_id='.$gid.' order by id limit 1')->fetchColumn(0);
				$strWhere .= " and A.dept IN ({$classId}) ";
				//$strWhere .= " and A.dept=".$cid." ";
			}else{
				$classId  = $cid;
				$strWhere .= " and A.dept={$classId} ";
			}
		...
		foreach($student as $rs){
			...
			//echo "select item_id,item_name,result from oa_zhsz_stamina_record where user_id='{$rs['userid']}' {$where} order by item_id";
	...
	case "remarks": #友朋寄语
		...
		#
		//$class = $pd->fetchRow(array('field'=>array('id','grade_id'),'table'=>'oa_zhsz_class','where'=>"class_master='{$_SESSION['uid']}' and grade_id<>0 and school={$_SESSION['school']}"));
		$cur_classid = $T->db->query("select id from oa_zhsz_class where class_master='{$_SESSION['uid']}' and grade_id<>0 and school={$_SESSION['school']} ")->fetchColumn(0);
		$where  = $strWhere;
		...
		/* if($from_id){  //消息跳转
			...
			$sql 	.= ' from oa_zhsz_remarks AS A ';
			$sql 	.= ' inner join oa_user_extend AS B ON A.user_id=B.userid inner join act_member AS C ON B.userid=C.id';
			$sql 	.= " where {$where} and B.dept=".$cur_classid." order by A.create_time DESC,A.flag_status";
		...
		foreach($study as $rs){
			...
			if($_SESSION['role_id']==4 || $_SESSION['role_id']==6){
				if(!empty($jiyu)){
					$str_total .= '<td class="tabtd2_L" style="text-align:center;vertical-align: middle;height: 100%;transform: translateY(0%);">'.$tips.'</td><td class="tabtd2_L" style="text-align:center;">'.$is_check.'</td><td class="tabtd" align="left">&nbsp;&nbsp;&nbsp;&nbsp;<a   id="shwtg" href="javascript:void(0);"class="glyphicon glyphicon-remove" title="删除寄语" onclick="delEvent('.$rs['id'].');" style="width:15px;overflow:hidden;margin:0 6px 0 10px ;">&nbsp;</a></td>';
				}else{
					$str_total .= '<td class="tabtd2_L" style="text-align:center;vertical-align: middle;height: 100%;transform: translateY(0%);">'.$tips.'</td><td class="tabtd2_L" style="text-align:center;">'.$is_check.'</td><td class="tabtd" align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/037.gif" /> <a style="padding-right: 1rem;text-align: center;display: inline-block;" class="ha_checkup" href="javascript:showReply('.$rs['id'].');">审核</a></td>';
				}
				$str_first = '<td class="tabtd"><input type="checkbox" name="id[]" id="id_'.$rs['id'].'" value="'.$rs['id'].'" />';
			}else{
				$str_first = '<td class="tabtd2_L" style="text-align:center;">'.$i.'</td>';
		...
		if($_SESSION['role_id']==4|| $_SESSION['role_id']==6){
			$is_show = 1;
			$str_show = '<td width="15%" class="tb_td tb_txt_bgc" style="text-align:center;">权限状态</td><td width="7%" class="tb_td tb_txt_bgc" style="text-align:center;">审核状态</td><td width="8%" class="tabtd" style="text-align:center;">操作选项</td>';
			$str_check = '<div style="background:#fff;margin-bottom:10px;"><select name="op" id="op"  class="ui-select"><option value="0">请选择操作</option><option value="1">审核通过</option><option value="2">取消审核</option></select><input type="submit" class="ha_selchoose" id="but_op" name="but_op" value="确定" /></div>';
	...
	case "user_manage": 
		...
		if($tid=="1"){
			...
			#班级
			$T->SetBlock("u_list","select A.*,B.class_name,C.grade_name from v_users as A left join oa_zhsz_class as B on B.id=A.dept left join oa_zhsz_grade as C on C.id=B.grade_id left join oa_zhsz_dept as D on D.id=A.dept where A.school=".$_SESSION['school'].$strWhere." and xj_status=1 limit ".(($p-1)*20).",20");
		}
		elseif($tid=="2"){
	...
	case "master_comment_manage": #班主任评语
		...
		if($offset<0) $offset = 0;
		$where  = $strWhere;
		$sql 	= "select A.id,A.class_id,A.term_id,A.class_name,A.master_id,A.master_name,A.content,A.create_time,B.year,B.term_name,C.truename ";
		$sql 	.= " from oa_zhsz_teacher_comment AS A ";
		...
		foreach($performance as $rs){
			...
			$master_i = $pd->fetchOne(array('field'=>'class_master','table'=>'oa_zhsz_class','where'=>'id="'.$rs['class_id'].'"'));
			if(empty($rs['master_name'])){
				$rs['master_name'] = $pd->fetchOne(array('field'=>'truename','table'=>'v_users','where'=>'id="'.$master_i.'"'));
			}
			$str .= '<tr id="tr_c_'.$rs['id'].'"><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['year'].$rs['term_name'].'</td><td class="tabtd">'.$rs['class_name'].'</td><td class="tabtd">'.$rs['master_name'].'</td><td class="tabtd">'.$str1.'</td><td class="tabtd ha_maedit">'.$str_edit.'</td>';
	...
	case "charts_height": #体质分析统计----身高
		...
		foreach($class as $rs){
			...
			if(!empty($boy[0])){
				...
				$girl_height .= '{value:"'.$boy[0]['height'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"girl"},';
			}else{
				$girl_height .= '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"girl"},';
			}
			
			if(!empty($boy[1])){
				//$boy_height .= '\''.$boy[1]['height'].'\',';
				$boy_height .= '{value:"'.$boy[1]['height'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"boy"},';
			}else{
				$boy_height .=  '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"boy"},';
			}
		}
		$T->Set("str_grade",$str_grade);
	...
	case "charts_weight": #体质分析统计----体重
		...
		foreach($class as $rs){
			...
			if(!empty($boy[0])){
				$girl_weight .= '{value:"'.$boy[0]['weight'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"girl"},';
			}else{
				$girl_weight .= '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"girl"},';
			}
			
			if(!empty($boy[1])){
				$boy_weight .= '{value:"'.$boy[1]['weight'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"boy"},';
			}else{
				$boy_weight .=  '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"boy"},';
			}
		}
		$T->Set("str_grade",$str_grade);
		$T->Set("str_year",$str_year);
	...
	case "charts_lung": #体质分析统计----肺活量
		...
		foreach($class as $rs){
			...
			if(!empty($boy[0])){
				$girl_lung .= '{value:"'.$boy[0]['lung'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"girl"},';
			}else{
				$girl_lung .= '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"girl"},';
			}
			
			if(!empty($boy[1])){
				$boy_lung .= '{value:"'.$boy[1]['lung'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"boy"},';
			}else{
				$boy_lung .=  '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",gender:"boy"},';
			}
		}
		$T->Set("str_grade",$str_grade);
		$T->Set("str_year",$str_year);
	...
	case "charts_height_classdis": #体质分析统计----身高表现
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_height_classdis&gender='.$gender.'&tyear='.$rs['year'].'&grade='.$gid.'&classid='.$classid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			...
			$str_class .= '<a href="./?t=charts_height_classdis&gender='.$gender.'&tyear='.$tyear.'&grade='.$gid.'&classid='.$rs['id'].'" target="main" '.$class_css.'><i>&nbsp;</i>'.$rs['class_name'].'</a>';
		}
		
		$str_height = '';
		$i = 1;
		if($gender=='boy'){
			$boy  = $pd->query("SELECT A.user_id,A.height,B.truename from oa_zhsz_physique as A INNER JOIN v_users as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取身高均值
			$name .='男生';
		}elseif($gender=='girl'){
			$boy  = $pd->query("SELECT A.user_id,A.height,B.truename from oa_zhsz_physique as A INNER JOIN v_users as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取身高均值
			$name .='女生';
		}
	...
	case "charts_height_class": #体质分析统计----身高段人数分布
		...
		$gender    = empty($_GET['gender'])?'':$_GET['gender'];//男生还是女生
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_height_class&gender='.$gender.'&tyear='.$rs['year'].'&grade='.$gid.'&classid='.$classid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			...
			$str_class .= '<a href="./?t=charts_height_class&gender='.$gender.'&tyear='.$tyear.'&grade='.$gid.'&classid='.$rs['id'].'" target="main" '.$class_css.'><i>&nbsp;</i>'.$rs['class_name'].'</a>';
		}
		
		$str_height = '';
		if($gender=='boy'){
			$height  = $pd->query("SELECT count(case WHEN A.height > 190 then 1 end)A,count(case WHEN 190 >=A.height and A.height >= 185 then 1 end)B,count(case WHEN 185 >A.height and A.height >= 180 then 1 end)C,count(case WHEN 180 >A.height and A.height >= 175 then 1 end)D,count(case WHEN 175 >A.height and A.height >= 170 then 1 end)E,count(case WHEN 170 >A.height and A.height >= 165 then 1 end)F,count(case WHEN 165 >A.height and A.height >= 160 then 1 end)G,count(case WHEN 160 >A.height and A.height >= 155 then 1 end)H,count(case WHEN 155 >A.height and A.height >= 150 then 1 end)I,count(case WHEN A.height < 150 then 1 end)J from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender ='男'")->fetchAll(PDO::FETCH_ASSOC);//获取身高段
			$name .='男生';
		}elseif($gender=='girl'){
			$height  = $pd->query("SELECT count(case WHEN A.height > 190 then 1 end)A,count(case WHEN 190 >=A.height and A.height >= 185 then 1 end)B,count(case WHEN 185 >A.height and A.height >= 180 then 1 end)C,count(case WHEN 180 >A.height and A.height >= 175 then 1 end)D,count(case WHEN 175 >A.height and A.height >= 170 then 1 end)E,count(case WHEN 170 >A.height and A.height >= 165 then 1 end)F,count(case WHEN 165 >A.height and A.height >= 160 then 1 end)G,count(case WHEN 160 >A.height and A.height >= 155 then 1 end)H,count(case WHEN 155 >A.height and A.height >= 150 then 1 end)I,count(case WHEN A.height < 150 then 1 end)J from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender ='女'")->fetchAll(PDO::FETCH_ASSOC);//获取身高段
			$name .='女生';
		}
	...
	case "charts_height_classper":     #体质分析统计----身高
		...
		$str_class = '';
		$str_grade = '';
		$str_analysis = '<dl style="color:#999;font-size:14px">';
		...
		foreach($year as $rs){
			...
			if($person)
				...
				$str_person .= ",";
			if($gender=='boy'){
				$class_avg  = $pd->query("SELECT round(AVG(A.height),2) as height from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$user['dept']} and A.term_year='{$rs['year']}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取班级身高均值
			}elseif($gender=='girl'){
				$class_avg  = $pd->query("SELECT round(AVG(A.height),2) as height from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$user['dept']} and A.term_year='{$rs['year']}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取班级身高均值
			}
			...
			if($class_avg)
				...
				$str_class .= ",";
			if($gender=='boy'){
				$school_avg  = $pd->query("SELECT round(AVG(A.height),2) as height from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept IN ({$classId}) and A.term_year='{$rs['year']}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取年级身高均值
			}elseif($gender=='girl'){
				$school_avg  = $pd->query("SELECT round(AVG(A.height),2) as height from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept IN ({$classId}) and A.term_year='{$rs['year']}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取年级身高均值
			}
			if($school_avg)
			...
			if(!empty($school_avg[0]['height'])&&!empty($class_avg[0]['height'])){
				if(@($person[0]['height'])<($class_avg[0]['height'])){
					$str = "<dd>".$rs['year']."学年身高<span style='color:#333'>小于</span>班级".$name."平均值,";
				}elseif(@$person[0]['height']==$class_avg[0]['height']){
					$str = "<dd>".$rs['year']."学年身高<span style='color:#333'>等于</span>班级".$name."平均值,";
				}else{
					$str = "<dd>".$rs['year']."学年身高<span style='color:#333'>大于</span>班级".$name."平均值,";
				}

				if(@$person[0]['height']<$school_avg[0]['height']){
					$str .= "<span style='color:#333'>小于</span>年级".$name."平均值</dd></br>";
				}elseif(@$person[0]['height']==$school_avg[0]['height']){
					$str .= "<span style='color:#333'>等于</span>年级".$name."平均值</dd></br>";
				}else{
					$str .= "<span style='color:#333'>大于</span>年级".$name."平均值</dd></br>";
				}
			}
		...
		$T->Set("str_class",$str_class);
		$T->Set("str_grade",$str_grade);
		$T->Set("str_analysis",$str_analysis);
		$T->display();
		$T->close();	
		$pd->close();
		unset($T);
	...
	case "charts_height_classlist":     #体质分析统计----身高分布列表
		...
		if($gender=='boy'){
			switch ($dataIndex) {
			case 0:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  A.height >= 190 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 1:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  190 >A.height and A.height >= 185 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 2:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  185 >A.height and A.height >= 180 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 3:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  180 >A.height and A.height >= 175 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 4:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  175 >A.height and A.height >= 170 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 5:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  170 >A.height and A.height >= 165 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 6:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  165 >A.height and A.height >= 160 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 7:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  160 >A.height and A.height >= 155 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 8:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  155 >A.height and A.height >= 150 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 9:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  A.height < 150 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			}
		}elseif($gender=='girl'){
			switch ($dataIndex) {
			case 0:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  A.height >= 190 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 1:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  190 >A.height and A.height >= 185 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 2:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  185 >A.height and A.height >= 180 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 3:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  180 >A.height and A.height >= 175 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 4:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  175 >A.height and A.height >= 170 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 5:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  170 >A.height and A.height >= 165 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 6:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  165 >A.height and A.height >= 160 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 7:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  160 >A.height and A.height >= 155 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 8:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  155 >A.height and A.height >= 150 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			case 9:
				$heightinfos  = $pd->query("SELECT A.height as height,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  A.height < 150 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取身高段学生信息
				break;
			}
		}
		...
		foreach($heightinfos as $k=>$rs){
			$rs['height'] = empty($rs['height'])?'':"{$rs['height']}";
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['height'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">身高(cm)</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_height_classlist&gender={$gender}&tyear={$tyear}&classid={$classid}&dataIndex={$dataIndex}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
	...
	case "charts_weight_classdis": #体质分析统计----体重表现
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_weight_classdis&gender='.$gender.'&tyear='.$rs['year'].'&grade='.$gid.'&classid='.$classid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			...
			$str_class .= '<a href="./?t=charts_weight_classdis&gender='.$gender.'&tyear='.$tyear.'&grade='.$gid.'&classid='.$rs['id'].'" target="main" '.$class_css.'><i>&nbsp;</i>'.$rs['class_name'].'</a>';
		}
		
		$str_weight = '';
		$i = 1;
		if($gender=='boy'){
			$boy  = $pd->query("SELECT A.user_id,A.weight,B.truename from oa_zhsz_physique as A INNER JOIN v_users as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取体重均值
			$name.='男生';
		}elseif($gender=='girl'){
			$boy  = $pd->query("SELECT A.user_id,A.weight,B.truename from oa_zhsz_physique as A INNER JOIN v_users as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取体重均值
			$name.='女生';
		}
	...
	case "charts_weight_class": #体质分析统计----体重段人数分布
		...
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		$classid    = empty($_GET['classid'])?'':$_GET['classid'];//班级id
		$gender    = empty($_GET['gender'])?'':$_GET['gender'];//性别
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_weight_class&gender='.$gender.'&tyear='.$rs['year'].'&grade='.$gid.'&classid='.$classid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			...
			$str_class .= '<a href="./?t=charts_weight_class&gender='.$gender.'&tyear='.$tyear.'&grade='.$gid.'&classid='.$rs['id'].'" target="main" '.$class_css.'><i>&nbsp;</i>'.$rs['class_name'].'</a>';
		}
		
		$str_weight = '';
		if($gender=='boy'){
			$weight  = $pd->query("SELECT count(case WHEN A.weight > 80 then 1 end)A,count(case WHEN 80 >=A.weight and A.weight >= 75 then 1 end)B,count(case WHEN 75 >A.weight and A.weight >= 70 then 1 end)C,count(case WHEN 70 >A.weight and A.weight >= 65 then 1 end)D,count(case WHEN 65 >A.weight and A.weight >= 60 then 1 end)E,count(case WHEN 60 >A.weight and A.weight >= 55 then 1 end)F,count(case WHEN 55 >A.weight and A.weight >= 50 then 1 end)G,count(case WHEN 50 >A.weight and A.weight >= 45 then 1 end)H,count(case WHEN 45 >A.weight and A.weight >= 40 then 1 end)I,count(case WHEN A.weight < 40 then 1 end)J from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取体重段
			$name.='男生';

		}elseif($gender=='girl'){
			$weight  = $pd->query("SELECT count(case WHEN A.weight > 80 then 1 end)A,count(case WHEN 80 >=A.weight and A.weight >= 75 then 1 end)B,count(case WHEN 75 >A.weight and A.weight >= 70 then 1 end)C,count(case WHEN 70 >A.weight and A.weight >= 65 then 1 end)D,count(case WHEN 65 >A.weight and A.weight >= 60 then 1 end)E,count(case WHEN 60 >A.weight and A.weight >= 55 then 1 end)F,count(case WHEN 55 >A.weight and A.weight >= 50 then 1 end)G,count(case WHEN 50 >A.weight and A.weight >= 45 then 1 end)H,count(case WHEN 45 >A.weight and A.weight >= 40 then 1 end)I,count(case WHEN A.weight < 40 then 1 end)J from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取体重段
			$name.='女生';
		}
	...
	case "charts_weight_classper":     #体质分析统计----体重
		...
		foreach($year as $rs){
			...
			if($person)
				...
				$str_person .= ",";
			if($gender=='boy'){
				$class_avg  = $pd->query("SELECT round(AVG(A.weight),2) as weight from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$user['dept']} and A.term_year='{$rs['year']}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取班级体重均值
			}elseif($gender=='girl'){
				$class_avg  = $pd->query("SELECT round(AVG(A.weight),2) as weight from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$user['dept']} and A.term_year='{$rs['year']}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取班级体重均值
			}
			...
			if($gender=='boy'){
				$school_avg  = $pd->query("SELECT round(AVG(A.weight),2) as weight from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept IN ({$classId}) and A.term_year='{$rs['year']}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取年级体重均值
			}elseif($gender=='girl'){
				$school_avg  = $pd->query("SELECT round(AVG(A.weight),2) as weight from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept IN ({$classId}) and A.term_year='{$rs['year']}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取年级体重均值
			}
			...
			if($school_avg)
				...
				$str_grade .= ",";
			if(!empty($school_avg[0]['weight'])&&!empty($class_avg[0]['weight'])){
				if(@($person[0]['weight'])<($class_avg[0]['weight'])){
					$str = "<dd>".$rs['year']."学年体重<span style='color:#333'>小于</span>班级".$name."平均值,";
				}elseif(@$person[0]['weight']==$class_avg[0]['weight']){
					$str = "<dd>".$rs['year']."学年体重<span style='color:#333'>等于</span>班级".$name."平均值,";
				}else{
					$str = "<dd>".$rs['year']."学年体重<span style='color:#333'>大于</span>班级".$name."平均值,";
				}

				if(@$person[0]['weight']<$school_avg[0]['weight']){
					$str .= "<span style='color:#333'>小于</span>年级".$name."平均值</dd></br>";
				}elseif(@$person[0]['weight']==$school_avg[0]['weight']){
					$str .= "<span style='color:#333'>等于</span>年级".$name."平均值</dd></br>";
				}else{
					$str .= "<span style='color:#333'>大于</span>年级".$name."平均值</dd></br>";
				}
	...
	case "charts_weight_classlist":     #体质分析统计----体重分布列表
		...
		if($gender=='boy'){
			switch ($dataIndex) {
			case 0:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  A.weight > 80 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 1:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  80 >=A.weight and A.weight >= 75 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 2:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  75 >A.weight and A.weight >= 70 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 3:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  70 >A.weight and A.weight >= 65 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 4:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  65 >A.weight and A.weight >= 60 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 5:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  60 >A.weight and A.weight >= 55 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 6:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  55 >A.weight and A.weight >= 50 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 7:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  50 >A.weight and A.weight >= 45 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 8:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  45 >A.weight and A.weight >= 40 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 9:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  A.weight < 40 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			...
			switch ($dataIndex) {
			case 0:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  A.weight > 80 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 1:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  80 >=A.weight and A.weight >= 75 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 2:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  75 >A.weight and A.weight >= 70 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 3:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  70 >A.weight and A.weight >= 65 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 4:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  65 >A.weight and A.weight >= 60 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 5:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  60 >A.weight and A.weight >= 55 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 6:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  55 >A.weight and A.weight >= 50 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 7:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  50 >A.weight and A.weight >= 45 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 8:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  45 >A.weight and A.weight >= 40 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 9:
				$weightinfos  = $pd->query("SELECT A.weight as weight,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  A.weight < 40 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
		...
		foreach($weightinfos as $k=>$rs){
			$rs['weight'] = empty($rs['weight'])?'':"{$rs['weight']}";
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['weight'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">体重(kg)</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_weight_classlist&gender={$gender}&tyear={$tyear}&classid={$classid}&dataIndex={$dataIndex}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
	...
	case "charts_bmi_classper":     #体质分析统计----BMI
		...
		foreach($year as $rs){
			...
			if((!empty($school_avg[0]['weight'])) && (!empty($school_avg[0]['height']))){
				if(@$bmi<$bmic){
					$str = "<dd>".$rs['year']."学年BMI<span style='color:#333'>小于</span>班级平均值,";
				}elseif(@$bmi==$bmic){
					$str = "<dd>".$rs['year']."学年BMI<span style='color:#333'>等于</span>班级平均值,";
				}else{
					$str = "<dd>".$rs['year']."学年BMI<span style='color:#333'>大于</span>班级平均值,";
				}

				if(@$bmi<$bmis){
					$str .= "<span style='color:#333'>小于</span>年级平均值</dd></br>";
				}elseif(@$bmi==$bmis){
					$str .= "<span style='color:#333'>等于</span>年级平均值</dd></br>";
				}else{
					$str .= "<span style='color:#333'>大于</span>年级平均值</dd></br>";
				}
			}
	...
	case "charts_bmi_classlist":     #体质分析统计----BMI分布列表
		...
		switch ($dataIndex) {
			case 0:
				$bmiinfos  = $pd->query("SELECT Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) as bmi,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) < 18.5 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取BMI段学生信息
				break;
			case 1:
				$bmiinfos  = $pd->query("SELECT Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) as bmi,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  24.0 >Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) and Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) >= 18.5 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取BMI段学生信息
				break;
			case 2:
				$bmiinfos  = $pd->query("SELECT Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) as bmi,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  28.0 >Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) and Cast(A.weight/A.height/A.height*10000 as Dec(4,1)) >= 24.0 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取BMI段学生信息
				break;
			case 3:
		...
		foreach($bmiinfos as $k=>$rs){
			$rs['bmi'] = empty($rs['bmi'])?'':"{$rs['bmi']}";
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['bmi'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">BMI</td><td class="tabtd" width="60%">家庭地址</td></tr>';
	...
	case "charts_lung_classdis": #体质分析统计----肺活量表现
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_lung_classdis&gender='.$gender.'&tyear='.$rs['year'].'&grade='.$gid.'&classid='.$classid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			...
			$str_class .= '<a href="./?t=charts_lung_classdis&gender='.$gender.'&tyear='.$tyear.'&grade='.$gid.'&classid='.$rs['id'].'" target="main" '.$class_css.'><i>&nbsp;</i>'.$rs['class_name'].'</a>';
		}
		...
		if($gender=='boy'){
			$name.='男生';
			$boy  = $pd->query("SELECT A.user_id,A.vc,B.truename from oa_zhsz_physique as A INNER JOIN v_users as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取体重均值
		}elseif($gender=='girl'){
			$name.='女生';
			$boy  = $pd->query("SELECT A.user_id,A.vc,B.truename from oa_zhsz_physique as A INNER JOIN v_users as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取体重均值
		}
	...
	case "charts_lung_class": #体质分析统计----体重段人数分布
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_lung_class&gender='.$gender.'&tyear='.$rs['year'].'&grade='.$gid.'&classid='.$classid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			...
			$str_class .= '<a href="./?t=charts_lung_class&gender='.$gender.'&tyear='.$tyear.'&grade='.$gid.'&classid='.$rs['id'].'" target="main" '.$class_css.'><i>&nbsp;</i>'.$rs['class_name'].'</a>';
		}
		
		$str_lung = '';
		if($gender=='boy'){
			$name.='男生';
			$lung  = $pd->query("SELECT count(case WHEN A.vc > 6000 then 1 end)A,count(case WHEN 6000 >=A.vc and A.vc >= 5000 then 1 end)B,count(case WHEN 5000 >A.vc and A.vc >= 4000 then 1 end)C,count(case WHEN 4000 >A.vc and A.vc >= 3000 then 1 end)D,count(case WHEN 3000 >A.vc and A.vc >= 2000 then 1 end)E,count(case WHEN A.vc < 2000 then 1 end)F from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取体重段
		}elseif($gender=='girl'){
			$name.='女生';
			$lung  = $pd->query("SELECT count(case WHEN A.vc > 6000 then 1 end)A,count(case WHEN 6000 >=A.vc and A.vc >= 5000 then 1 end)B,count(case WHEN 5000 >A.vc and A.vc >= 4000 then 1 end)C,count(case WHEN 4000 >A.vc and A.vc >= 3000 then 1 end)D,count(case WHEN 3000 >A.vc and A.vc >= 2000 then 1 end)E,count(case WHEN A.vc < 2000 then 1 end)F from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取体重段
		}
	...
	case "charts_lung_classper":     #体质分析统计----肺活量
		...
		foreach($year as $rs){
			...
			if($person)
				...
				$str_person .= ",";
			if($gender=='boy'){
				$class_avg  = $pd->query("SELECT round(AVG(A.vc),2) as vc from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$user['dept']} and A.term_year='{$rs['year']}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取班级体重均值
			}elseif($gender=='girl'){
				$class_avg  = $pd->query("SELECT round(AVG(A.vc),2) as vc from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$user['dept']} and A.term_year='{$rs['year']}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取班级体重均值
			}
			...
			if($class_avg)
				...
				$str_class .= ",";
			if($gender=='boy'){
				$school_avg  = $pd->query("SELECT round(AVG(A.vc),2) as vc from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept IN ({$classId}) and A.term_year='{$rs['year']}' and B.gender='男'")->fetchAll(PDO::FETCH_ASSOC);//获取年级体重均值
			}elseif($gender=='girl'){
				$school_avg  = $pd->query("SELECT round(AVG(A.vc),2) as vc from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept IN ({$classId}) and A.term_year='{$rs['year']}' and B.gender='女'")->fetchAll(PDO::FETCH_ASSOC);//获取年级体重均值
			}
			...
			if($school_avg)
				...
				$str_grade .= ",";
			if(!empty($school_avg[0]['vc'])&&!empty($class_avg[0]['vc'])){
				if(@($person[0]['vc'])<($class_avg[0]['vc'])){
					$str = "<dd>".$rs['year']."学年肺活量<span style='color:#333'>小于</span>班级".$name."平均值,";
				}elseif(@$person[0]['vc']==$class_avg[0]['vc']){
					$str = "<dd>".$rs['year']."学年肺活量<span style='color:#333'>等于</span>班级".$name."平均值,";
				}else{
					$str = "<dd>".$rs['year']."学年肺活量<span style='color:#333'>大于</span>班级".$name."平均值,";
				}

				if(@$person[0]['vc']<$school_avg[0]['vc']){
					$str .= "<span style='color:#333'>小于</span>年级".$name."平均值</dd></br>";
				}elseif(@$person[0]['vc']==$school_avg[0]['vc']){
					$str .= "<span style='color:#333'>等于</span>年级".$name."平均值</dd></br>";
				}else{
					$str .= "<span style='color:#333'>大于</span>年级".$name."平均值</dd></br>";
				}
	...
	case "charts_lung_classlist":     #体质分析统计----肺活量分布列表
		...
		if($gender=='boy'){
			switch ($dataIndex) {
			case 0:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  A.vc > 6000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 1:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  6000 >=A.vc and A.vc >= 5000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 2:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  5000 >A.vc and A.vc >= 4000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 3:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  4000 >A.vc and A.vc >= 3000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 4:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  3000 >A.vc and A.vc >= 2000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 5:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='男' and  A.vc<2000  limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			}
		}elseif($gender=='girl'){
			switch ($dataIndex) {
			case 0:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  A.vc > 6000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 1:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  6000 >=A.vc and A.vc >= 5000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 2:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  5000 >A.vc and A.vc >= 4000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 3:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  4000 >A.vc and A.vc >= 3000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 4:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  3000 >A.vc and A.vc >= 2000 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			case 5:
				$weightinfos  = $pd->query("SELECT A.vc as vc,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid  WHERE B.dept={$classid} and A.term_year='{$tyear}' and B.gender='女' and  A.vc<2000  limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取体重段学生信息
				break;
			}
		}
		...
		$str = '';
		foreach($weightinfos as $k=>$rs){
			$rs['vc'] = empty($rs['vc'])?'':"{$rs['vc']}";
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['vc'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">肺活量</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_lung_classlist&gender={$gender}&tyear={$tyear}&classid={$classid}&dataIndex={$dataIndex}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
	...
	case "charts_sight_classper":     #体质分析统计----视力
		...
		foreach($year as $rs){
			...
			if($school_avg)
				...
				$str_grade .= ",";
			if(!empty($school_avg[0][$eye_type])&&!empty($class_avg[0][$eye_type])){
				if(@($person[0][$eye_type])<($class_avg[0][$eye_type])){
					$str = "<dd>".$rs['year']."学年".$name."视力<span style='color:#333'>小于</span>班级平均值,";
				}elseif(@$person[0][$eye_type]==$class_avg[0][$eye_type]){
					$str = "<dd>".$rs['year']."学年".$name."视力<span style='color:#333'>等于</span>班级平均值,";
				}else{
					$str = "<dd>".$rs['year']."学年".$name."视力<span style='color:#333'>大于</span>班级平均值,";
				}

				if(@$person[0][$eye_type]<$school_avg[0][$eye_type]){
					$str .= "<span style='color:#333'>小于</span>年级平均值</dd></br>";
				}elseif(@$person[0][$eye_type]==$school_avg[0][$eye_type]){
					$str .= "<span style='color:#333'>等于</span>年级平均值</dd></br>";
				}else{
					$str .= "<span style='color:#333'>大于</span>年级平均值</dd></br>";
				}
	...
	case "charts_sight_classlist":     #体质分析统计----视力分布列表
		...
		if($eye_type=='left_eye'){
			...
			switch ($dataIndex) {
			case 0:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.left_eye >= 5.0 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 1:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  5.0 >A.left_eye and A.left_eye >= 4.8 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 2:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.8 >A.left_eye and A.left_eye >= 4.6 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 3:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.6 >A.left_eye and A.left_eye >= 4.4 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 4:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.4 >A.left_eye and A.left_eye >= 4.2 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 5:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.2 >A.left_eye and A.left_eye >= 4.0 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 6:
				$sightinfos  = $pd->query("SELECT A.left_eye as left_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.0 >A.left_eye limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			}
		}elseif($eye_type=='right_eye'){
			$name = '右眼';
			switch ($dataIndex) {
			case 0:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.right_eye >= 5.0 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 1:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  5.0 >A.right_eye and A.right_eye >= 4.8 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 2:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.8 >A.right_eye and A.right_eye >= 4.6 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 3:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.6 >A.right_eye and A.right_eye >= 4.4 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 4:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.4 >A.right_eye and A.right_eye >= 4.2 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 5:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.2 >A.right_eye and A.right_eye >= 4.0 limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			case 6:
				$sightinfos  = $pd->query("SELECT A.right_eye as right_eye,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  4.0 >A.right_eye limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取视力段学生信息
				break;
			}
		}
		...
		foreach($sightinfos as $k=>$rs){
			$rs[$eye_type] = empty($rs[$eye_type])?'':"{$rs[$eye_type]}";
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs[$eye_type].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">视力('.$name.')</td><td class="tabtd" width="60%">家庭地址</td></tr>';
	...
	case "charts_oral_class": #体质分析统计----口腔
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_oral_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_oral_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.mouth=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
			}else{
				$str_abnomal .= '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .=  '{value:"",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
			}
		...
		$T->Set("str_year",$str_year);
		$T->Set("str_class",$str_class);
		$T->Set("str_nomal",$str_nomal);
		$T->Set("str_abnomal",$str_abnomal);
		break;
		
	case "charts_eyes_class": #体质分析统计----眼科
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_eyes_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_eyes_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.yanke=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_spleen_class": #体质分析统计----脾
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_spleen_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_spleen_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.spleen=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_blood_class": #体质分析统计----血压
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_blood_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_blood_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.blood=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_surgical_class": #体质分析统计----外科常规
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_surgical_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_surgical_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.wkcg=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_xf_class": #体质分析统计----心肺听诊
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_xf_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_xf_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.xf_ting=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_bravery_class": #体质分析统计----胆
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_bravery_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_bravery_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.guts=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_liver_class": #体质分析统计----肝
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];
		...
		foreach($grade as $rs){
			...
			if($gid == $rs['id'])
				$class_css = ' class="active"';
			$str_grade .= '<li'.$class_css.'><a href="./?t=charts_liver_class&grade='.$rs['id'].'" target="main">'.$rs['grade_name'].'年级</a></li>';	
		}
		...
		foreach($year as $rs){
			...
			if($tyear == $rs['year'])
				$year_css = ' class="da_current"';
			$str_year .= '<a href="./?t=charts_liver_class&tyear='.$rs['year'].'&grade='.$gid.'" target="main" '.$year_css.'><i>&nbsp;</i>'.$rs['year'].'</a>';	
		}
		...
		foreach($class as $rs){
			$str_class .= '\''.$rs['class_name'].'\',';
			$boy  = $pd->query("SELECT count(*) as total,count(case WHEN A.liver=\"正常\" then 1 end)nomal from oa_zhsz_physique as A INNER JOIN oa_user_extend as B on A.user_id=B.userid WHERE B.dept={$rs['id']} and A.term_year='{$tyear}'")->fetchAll(PDO::FETCH_ASSOC);//获取口腔人数
			if(!empty($boy[0])){
				$abnomal = $boy[0]['total'] - $boy[0]['nomal'];
				$str_abnomal .= '{value:"'.$abnomal.'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"abnomal"},';
				$str_nomal .= '{value:"'.$boy[0]['nomal'].'",classid:"'.$rs['id'].'",grade:"'.$gid.'",tyear:"'.$tyear.'",isnomal:"nomal"},';
	...
	case "charts_liver_classlist":     #体质分析统计----肝分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		$classid    = empty($_GET['classid'])?'':$_GET['classid'];//班级id
		$isnomal    = empty($_GET['isnomal'])?'':$_GET['isnomal'];//是否正常
		$count    = empty($_GET['count'])?'':$_GET['count'];//该分布段人数
		...
		if($isnomal=='nomal'){
			$liverinfos  = $pd->query("SELECT A.liver as liver,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.liver = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取肝段学生信息
		}else{
			$liverinfos  = $pd->query("SELECT A.liver as liver,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.liver <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取肝段学生信息
		}
		...
		$str = '';
		foreach($liverinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['liver'] = empty($rs['liver'])?'':"{$rs['liver']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['liver'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">肝</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_liver_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_bravery_classlist":     #体质分析统计----胆分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$braveryinfos  = $pd->query("SELECT A.guts as bravery,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.guts = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取胆段学生信息
		}else{
			$braveryinfos  = $pd->query("SELECT  A.guts as bravery,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.guts <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取胆段学生信息
		}
		...
		$str = '';
		foreach($braveryinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['bravery'] = empty($rs['bravery'])?'':"{$rs['bravery']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['bravery'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">胆</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_bravery_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_xf_classlist":     #体质分析统计---心肺听诊分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$xfinfos  = $pd->query("SELECT A.xf_ting as xf,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.xf_ting = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取心肺听诊段学生信息
		}else{
			$xfinfos  = $pd->query("SELECT A.xf_ting as xf,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.xf_ting <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取心肺听诊段学生信息
		}
		...
		$str = '';
		foreach($xfinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['xf'] = empty($rs['xf'])?'':"{$rs['xf']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['xf'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">心扉听诊</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_xf_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_surgical_classlist":     #体质分析统计----外科常规分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$surgicalinfos  = $pd->query("SELECT A.wkcg as surgical,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.wkcg = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取外科常规段学生信息
		}else{
			$surgicalinfos  = $pd->query("SELECT A.wkcg as surgical,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.wkcg <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取外科常规段学生信息
		}
		...
		$str = '';
		foreach($surgicalinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['surgical'] = empty($rs['surgical'])?'':"{$rs['surgical']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['surgical'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">外科常规</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_surgical_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_blood_classlist":     #体质分析统计---血压分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$bloodinfos  = $pd->query("SELECT A.blood as blood,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.blood = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取血压段学生信息
		}else{
			$bloodinfos  = $pd->query("SELECT A.blood as blood,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.blood <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取血压段学生信息
		}
		...
		$str = '';
		foreach($bloodinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['blood'] = empty($rs['blood'])?'':"{$rs['blood']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['blood'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">血压</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_blood_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_spleen_classlist":     #体质分析统计----脾分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$spleeninfos  = $pd->query("SELECT A.spleen as spleen,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.spleen = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取脾段学生信息
		}else{
			$spleeninfos  = $pd->query("SELECT A.spleen as spleen,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.spleen <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取脾段学生信息
		}
		...
		$str = '';
		foreach($spleeninfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['spleen'] = empty($rs['spleen'])?'':"{$rs['spleen']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['spleen'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">脾</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_spleen_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_eyes_classlist":     #体质分析统计----眼科分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$eyesinfos  = $pd->query("SELECT A.yanke as eyes,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.yanke = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取眼科段学生信息
		}else{
			$eyesinfos  = $pd->query("SELECT A.yanke as eyes,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.yanke <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取眼科段学生信息
		}
		...
		$str = '';
		foreach($eyesinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['eyes'] = empty($rs['eyes'])?'':"{$rs['eyes']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['eyes'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">眼科</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_eyes_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		...
		case "charts_oral_classlist":     #体质分析统计----口腔分布列表
	 	$p=isset($_GET["p"])?$_GET["p"]:"1"; 	
		$p = ($p-1)*20;
		$gid = empty($_GET['grade'])?1:intval($_GET['grade']);//年级
		$tyear    = empty($_GET['tyear'])?'':$_GET['tyear'];//哪一年的数据
		...
		if($isnomal=='nomal'){
			$oralinfos  = $pd->query("SELECT A.mouth as oral,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.mouth = '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取口腔段学生信息
		}else{
			$oralinfos  = $pd->query("SELECT A.mouth as oral,C.truename as truename,B.gender as gender,B.address as address  from oa_zhsz_physique as A INNER JOIN oa_user_extend as B  on A.user_id=B.userid INNER JOIN v_users as C on B.userid=C.userid WHERE B.dept={$classid} and A.term_year='{$tyear}' and  A.mouth <> '正常' limit {$p},20")->fetchAll(PDO::FETCH_ASSOC);//获取口腔段学生信息
		}
		...
		$str = '';
		foreach($oralinfos as $k=>$rs){
			$rs['truename'] = empty($rs['truename'])?'':"{$rs['truename']}";
			$rs['gender'] = empty($rs['gender'])?'':"{$rs['gender']}";
			$rs['oral'] = empty($rs['oral'])?'':"{$rs['oral']}";
			$rs['address'] = empty($rs['address'])?'':"{$rs['address']}";
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['gender'].'</td><td class="tabtd">'.$rs['oral'].'</td><td class="tabtd">'.$rs['address'].'</td></tr>';
		}
		
		$str_tr='<tr class="tabtitle" height="26"><td class="tabtd" width="20%">姓名</td><td class="tabtd" width="10%">性别</td><td class="tabtd" width="10%">口腔</td><td class="tabtd" width="60%">家庭地址</td></tr>';
		
		$str_tr .= $str;
		$p=isset($_GET["p"])?$_GET["p"]:"0";  	   
		if($p<1)$p=1;
		$page=getPageHtml_bt($count,20,$p,"&t=charts_oral_classlist&grade={$gid}&isnomal={$isnomal}&tyear={$tyear}&classid={$classid}&isnomal={$isnomal}&count={$count}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
    ...
    /* case "points_select_manage":
		...
		$str_report .= '<td width="6%" class="tabtd" style="color:#FF0000;">积分汇总</td>';
		$report_menu  = $pd->query("SELECT * from  oa_zhsz_report where parent_no='' and school={$_SESSION['school']} order by order_value")->fetchAll(PDO::FETCH_ASSOC);//下拉框
		$str_option = '<option value="">全部</option>';
		...
		if(!empty($user_name)){
			$strWhere .= " and C.truename LIKE'{$user_name}%' ";
		}
		if($report_id){
			$strWhere .=" and B.subject_son_id=".$report_id." ";
		}
		//显示信息查询
		$where  = $strWhere;
		$sql 	= 'select A.id,A.class_name,A.class_master,A.grade_id,B.point_sum,C.truename';
		$sql 	.= ' from oa_zhsz_class AS A left join v_users AS C ON C.dept=A.id left join oa_zhsz_point AS B ON B.uid=C.id';
		$sql 	.= " where {$where} ";
		$sql 	.= ' order by A.grade_id,A.id ';
		$sql 	.= " limit {$offset},20";
		$class  = $pd->query($sql)->fetchAll(PDO::FETCH_ASSOC);
		...
		foreach($class as $rs){
			$rs['grade_name']  = $pd->fetchOne(array('field'=>'grade_name','table'=>'oa_zhsz_grade','where'=>"id={$rs['grade_id']}"));
			$master_name = $pd->fetchOne(array('field'=>'truename','table'=>'v_users','where'=>"id='{$rs['class_master']}' "));
			#查询分类
			$report  = $pd->query("SELECT * from  oa_zhsz_report where parent_no='{$report_id}' and school={$_SESSION['school']} order by order_value")->fetchAll(PDO::FETCH_ASSOC);
			
			$count_sum = '';
			echo $report_id;
			foreach($report as $rs1){
				$count_s = $pd->fetchOne(array('field'=>'point_sum','table'=>'oa_zhsz_point','where'=>"{$report_where}='{$rs1['id']}' "));
				echo $report_where."&nbsp;".$rs1['id']."&nbsp;".$count_s;
				echo "<br/>";
				$count_s += $count_s;
				$count_sum .= '<td class="tabtd">'.$count_s.'</td>';
			}
			
			$str = '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['grade_name'].$rs['class_name'].'</td><td class="tabtd">'.$master_name.'</td><td class="tabtd">'.$count_sum.'</td></tr>';
		}
    ...
    case "points_select_manage":
		...
		$strWhere .= ' and A.grade_id<>0';
		$where  = $strWhere;
		$sql 	= 'select A.id,A.class_name,A.grade_id,A.class_master,C.truename,C.id as Uid';
		$sql 	.= ' from oa_zhsz_class AS A left join v_users AS C ON C.dept=A.id';
		$sql 	.= " where {$where} ";
		$sql 	.= ' order by A.grade_id,A.id ';
		$sql 	.= " limit {$offset},20";
		//班级查询
		...
		$where  = $strWhere;
		$sql_C 	= 'select A.id,A.class_name,A.grade_id,A.class_master,C.truename,C.id as Uid';
		$sql_C 	.= ' from oa_zhsz_class AS A left join v_users AS C ON C.dept=A.id';
		$sql_C 	.= " where {$where} ";
		
		$scoreN = $pd->query($sql_C)->fetchAll(PDO::FETCH_ASSOC);
		$rc = count($scoreN);
		...
		foreach($class as $rs){
			...
			$points_nums = 0;
			foreach($report as $rs1){
				$custom_total  = $pd->fetchOne(array('field'=>'count(*)','table'=>'oa_zhsz_report_custom','where'=>"{$report_where}={$rs1['id']}  and uid='{$rs['Uid']}' and term_id={$tid}"));   //已填写数量
				$point = $pd->fetchOne(array('field'=>'point_value','table'=>'oa_zhsz_report','where'=>"id={$rs1['id']}"));
				if($report_id){
					$points = $custom_total*$point;
				}else{
					$report_t  = $pd->query("SELECT * from  oa_zhsz_report where parent_no='' and school={$_SESSION['school']} order by order_value")->fetchAll(PDO::FETCH_ASSOC);
					foreach($report_t as $rs2){
						$custom_total = $pd->fetchOne(array('field'=>'point_sum','table'=>'oa_zhsz_point','where'=>'subject_id='.$rs1['id'].' and uid="'.$rs['Uid'].'" and  term_id='.$tid.''));
					}
					$points = $custom_total;
				}
				#汇总
				...
				$points_nums += $points;
				$str_costum .= '<td class="tabtd">'.$points.'</td>';
			}
			$master_name = $pd->fetchOne(array('field'=>'truename','table'=>'v_users','where'=>"id='{$rs['class_master']}' "));
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['grade_name'].$rs['class_name'].'</td><td class="tabtd">'.$master_name.'</td>'.$str_costum.'<td class="tabtd">'.$points_nums.'</td></tr>';
		}
		$page=getPageHtml_bt($rc,20,$p,"&t=points_select_manage&term_page={$tid}&report_page={$report_id}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
    ...
    case "self_point":
		...
		$sql 	= 'select A.id,A.class_name,A.grade_id,A.class_master,C.truename,C.id as Uid';
		$sql 	.= ' from oa_zhsz_class AS A left join v_users AS C ON C.dept=A.id ';
		$sql 	.= " where {$where} and C.id='".$_SESSION['uid']."'";
		//$sql 	.= ' order by A.grade_id,A.id ';
		$sql 	.= " limit {$offset},20";		
		//班级查询
		$class  = $pd->query($sql)->fetchAll(PDO::FETCH_ASSOC);
      // echo $sql;
		...
		$sql_C 	= 'select A.id,A.class_name,A.grade_id,A.class_master,C.truename,C.id as Uid';
		$sql_C 	.= ' from oa_zhsz_class AS A left join v_users AS C ON C.dept=A.id';
		$sql_C 	.= " where {$where} and C.id='".$_SESSION['uid']."'";
		...
		foreach($class as $rs){
			...
			$points_nums = 0;
			foreach($report as $rs1){
				$custom_total  = $pd->fetchOne(array('field'=>'count(*)','table'=>'oa_zhsz_report_custom','where'=>"{$report_where}={$rs1['id']}  and uid='".$_SESSION['uid']."' and term_id={$tid}"));   //已填写数量
				$point = $pd->fetchOne(array('field'=>'point_value','table'=>'oa_zhsz_report','where'=>"id={$rs1['id']}"));
				if($report_id){
					$points = $custom_total*$point;
				}else{
					$report_t  = $pd->query("SELECT * from  oa_zhsz_report where parent_no='' and school={$_SESSION['school']} order by order_value")->fetchAll(PDO::FETCH_ASSOC);
					foreach($report_t as $rs2){
						$custom_total = $pd->fetchOne(array('field'=>'point_sum','table'=>'oa_zhsz_point','where'=>'subject_id='.$rs1['id'].' and uid="'.$_SESSION['uid'].'" and  term_id='.$tid.''));
					}
					$points = $custom_total;
				}
				#汇总
		...
		$page=getPageHtml_bt($rc,20,$p,"&t=self_point&term_page={$tid}&report_page={$report_id}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
		$T->Set("term_id",$tid);		
		$T->Set("page",$page); 
		$T->Set("total",$rc); 
    ...
    case "report_select":
		...
		foreach($class as $rs){
			...
			$master_name = $pd->fetchOne(array('field'=>'truename','table'=>'v_users','where'=>"id='{$rs['class_master']}' "));
			$str .= '<tr><td class="tabtd">'.$rs['truename'].'</td><td class="tabtd">'.$rs['grade_name'].$rs['class_name'].'</td><td class="tabtd">'.$master_name.'</td>'.$str_costum.'</tr>';
		}
		$page=getPageHtml_bt($rc,20,$p,"&t=points_select_manage&term_page={$tid}&report_page={$report_id}");
		$page = mb_convert_encoding($page, 'utf-8', 'gbk');
	...
	case "art_am":
	    ...
	    $str_menu = "";
		if($_SESSION['role_id']==4 || $_SESSION['role_id']==5){
			$str_menu = '<div class="boxtitle"><label class="current"><a href="javascript:void(0);"></a></label><span class="floatright"><img src="images/add.gif" class="padding5"/><a href="./?t=art_am">添加基本信息</a> &nbsp;| <img src="images/add.gif" class="padding5"/><a href="./?t=teacher_add">添加教师</a> &nbsp;| <img src="images/add.gif" class="padding5"/><a href="./?t=admin_add">添加管理员</a> &nbsp;| <img src="images/add.gif" class="padding5"/><a href="./?t=member_import">批量导入学生</a> &nbsp;| <img src="images/add.gif" class="padding5"/><a href="./?t=teacher_import">批量导入教师</a> &nbsp;</span></div>';
			$is_show = 1;
		}
		$T->Set("str_menu",$str_menu);
        break;	
