...
switch($_POST["tpl"]){
   case "get_usrs":
    ...
    case "get_addr_s":  #nouse
  		...
  		break;  
  	case "get_blog_cate":
      $result = $pd->query("select id,name,pid from push_category where push_type='".$_POST["push_type"]."' order by odx desc")->fetchAll(PDO::FETCH_ASSOC);  
      echo get_str($result,0,-1);	
  		break;
    case "get_blog_catename":
      $result = $pd->query("select id,name from push_category where push_type='".$_POST["push_type"]."' order by odx desc")->fetchAll(PDO::FETCH_ASSOC);  
      echo json_encode($result);	
  		break;
    case "get_recommendcate":
      $result = $pd->query("select id,name,pid from recommend_art_category order by odx desc")->fetchAll(PDO::FETCH_ASSOC);  
      echo get_str($result,0,-1);
      break; 
}		
