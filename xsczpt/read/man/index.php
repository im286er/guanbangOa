...
switch($qname){    
  case "recommend": 
    ...
    $rc= $T->db->query("select count(1) from `recommend_list` where ".$wh)->fetchColumn(0);
    $page=getPageHtml_bt($rc,15,$p,"&t=recommend&c=".$cid."&so=".$so);
    $T->Set("page",$page);
    $T->SetBlock("list","select al.id,al.title,al.see,al.comments,al.timestamp,al.state,al.uid,am.truename from `recommend_list` as al LEFT JOIN `act_member` as am ON am.id=al.uid where ".$wh." order by al.timestamp desc limit ".(($p-1)*15).",15",array('timestamp'),array('%Y-%m-%d %H:%M'));
    $T->Set("so",$so);
    break;
  case "active_status":
  ...
  case "topics":
    ...
    $wh="";
    if(!empty($so))$wh=" and name like '%".$so."%'";
    $rc= $T->db->query("select count(1) from `main_topic` where id>0".$wh)->fetchColumn(0);
    $page=getPageHtml_bt($rc,15,$p,"&t=topics&so=".$so);
    $T->Set("page",$page); 
    $T->SetBlock("list","select T.*,M.truename from `main_topic` T left join act_member M on M.id=T.uid where T.id>0".$wh." order by T.timestamp desc limit ".(($p-1)*15).",15");
    break;  
  case "arts":
    $p=isset($_GET["p"])?$_GET["p"]:"1"; 
    $so=isset($_GET["so"])?$_GET["so"]:"";
