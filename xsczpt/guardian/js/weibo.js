...
function load_comments(v,obj){  
  ...
  $.post("./srv/rdo.php", {tpl:"get_weibo_comments_next","id":v,"page":npage}, function (d, e) { 
     ...
     for(i=0;i<d1.length;i++){
        var redes = (d1[i].redes==null)?'':dl[i].redes;
        truename = (d1[i].truename==null)?'':d1[i].truename;
        up = (d1[i].up==null)?0:d1[i].up;
        bad = (d1[i].bad==null)?0:d1[i].bad;
        $(obj).parent().before('<fieldset><legend>'+truename+' '+utctime(d1[i].timestamp)+'  '+d1[i].floor+'楼</legend>\
   '+redes+ d1[i].des+'<small class=" pull-right"><code class="pull-right">   \
  <a onclick="comment('+d1[i].wid+','+d1[i].id+')" >回复</a> &nbsp;  \
  <a class="glyphicon glyphicon-hand-up" onclick=vote({tbl:"weibo_comments",do:"up",id:'+d1[i].id+'},this) title="赞">('+up+')</a>  &nbsp; \
  <a class="glyphicon glyphicon-thumbs-down" onclick=vote({tbl:"weibo_comments",do:"bad",id:'+d1[i].id+'},this) title="反对">('+bad+')</a> &nbsp; \
