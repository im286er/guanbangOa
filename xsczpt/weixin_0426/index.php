...
switch($qname){
	...
	case "report_custom_check":  //自定义评价审核
		...
		$str_main = '';
		$main = $pd->query("select A.* ,B.truename,C.dept from oa_zhsz_report_custom as A inner join act_member as B on(A.uid=B.id) inner join oa_user_extend as C on(B.id=C.userid) where {$where} order by create_time desc   limit {$offset},10")->fetchAll(PDO::FETCH_ASSOC);
		$i = 1;
		foreach($main as $rs){
	...
	case "report":  //自定义评价
		...
		foreach($main as $rs){
			...
			$str_main .= '<tr class="ms_info"><td class="tb_td" width="100">'.$rs['subject'].'</td><td class="tb_td" width="100">'.$rs['subject_son'].'</td><td class="tb_td" width="100">'.$grade.'</td><td class="tb_td" ><div class="ha_tacontent" width="500">'.$rs['content'].'</div></td></tr>';
		}
