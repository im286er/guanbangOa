...
switch($qname){
  ...
  $tl_url="http://uc.jssns.cn/?t=login&url=".url_base64_encode($_SERVER["REQUEST_URI"]);
	$T->Set("tl_url",$tl_url);
    break;
  case "actived":
...
case "famous":
  ...
  case "index":
      ...
      break;
  case "error":        
      $no=getcode($_GET["no"])?iconv('utf-8', 'gbk', $_GET["no"]):$_GET["no"];        
	    $name=getcode($_GET["name"])?iconv('utf-8', 'gbk', $_GET["name"]):$_GET["name"];        
	    $msg=getcode($_GET["msg"])?iconv('utf-8', 'gbk', $_GET["msg"]):$_GET["msg"];        
