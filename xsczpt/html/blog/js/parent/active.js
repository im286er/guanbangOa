...
function getChildData(){
  $("#period a[name='"+queryStr("per")+"']").addClass("active");
  $("#subject a[name='"+queryStr("sub")+"']").addClass("active");
  $("#addr a[name='"+queryStr("a")+"']").addClass("active");
  if(qaddr!="0"){
    $.post("./srv/rdo.php?", {tpl:"getaddr2","id":qaddr}, function (d, e) {         
        d1=JSON.parse(d);
        $("#addr1").html("<label><a name=0>街道：</a></label>");
		$("#addr1").append('<div class="nmlist periodList"></div');
        if(d1.length>0){           
          for(i=0;i<d1.length;i++){
            $("#addr1 div").append(' <a name="'+d1[i].id+'">'+d1[i].name+'</a> &nbsp;');
          }
          //event
...
//设置选择的项
var qperiod,qsubject,qaddr,qaddr1,qso;
function showCurSeleted(){ 
  qperiod=queryStr("per");
  qsubject=queryStr("sub");
  qaddr=queryStr("a");
  qaddr1=queryStr("a1");

  if(qperiod=="")qperiod="0";   
  if(qsubject=="")qsubject="0";
  if(qaddr=="")qaddr="0";
  if(qaddr1=="")qaddr1="0"; 
 
  /* if(qsubject!="0"){$("#subject a[name='"+qsubject+"']").addClass("active"); 
...
function showData2Page(d){    
  $("#period").html("<label><a name=0>学段：</a></label>");
  $("#period").append('<div class="nmlist periodList"></div');
  for(i=0;i<d.period.length;i++){
    $("#period div").append('<a name="'+d.period[i].id+'">'+d.period[i].name+'</a> &nbsp;');
  }
  
  $("#subject").html("<label><a name=0>学科：</a></label>");
  $("#subject").append('<div class="nmlist periodList"></div');
  for(i=0;i<d.subject.length;i++){
    $("#subject div").append('<a name="'+d.subject[i].id+'">'+d.subject[i].name+'</a> &nbsp;');
  }
  
  $("#addr").html("<label><a name=0>地区：</a></label>");
  $("#addr").append('<div class="nmlist periodList"></div');
  for(i=0;i<d.addr.length;i++){
    $("#addr div").append('<a name="'+d.addr[i].id+'">'+d.addr[i].name+'</a> &nbsp;');
  }

  $("#period a").bind("click",function(a,b){setQry(qperiod=$(this).attr("name"));});
  $("#subject a").bind("click",function(a,b){setQry(qsubject=$(this).attr("name"));});
  $("#addr a").bind("click",function(a,b){qaddr1=0;setQry(qaddr=$(this).attr("name"));});

}
//添加选择条件
function setQry(v){
  qry="&per="+qperiod+"&sub="+qsubject+"&a="+qaddr+"&a1="+qaddr1+"&so="+$("#so").val();
  toUrl(qry);
} 
